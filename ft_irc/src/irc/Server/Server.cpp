...
/*   Updated: 2022/06/13 08:53:44 by Leo Suardi       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace irc
{
	...
	vector< string > Server::m_parseCommand( const string &rawCommand )
	{
		...
		if (!command.empty() && command.back()[command.back().length() - 1] == '\r')
	...
	void	Server::m_parsePending( void )
	{
		...
		for (Iter it = m_pending.begin(); it != m_pending.end(); ++it)
		{
			...
			while ((found = it->second.find('\n')) != string::npos || it->second.size() == 500)
			{
				vector< string > cmd = m_parseCommand(it->second.substr(0, found));
				if (!cmd.empty())
					m_cmds[it->first].push(cmd);
				// Erase the command from the pending buffer
				it->second.erase(0, found + (found != string::npos));
			}
		}
	}
